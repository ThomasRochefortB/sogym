...
def main():
    ...
    def extract_run_directory(checkpoint_path):
        """Extracts the run directory from the checkpoint path."""
        return os.path.basename(os.path.dirname(os.path.dirname(checkpoint_path)))

    if args.restart_run:
        print(f"Using checkpoint path: {args.restart_run}")
        log_name = extract_run_directory(args.restart_run)
        print(f"Extracted run directory: {log_name}")

        config_file = f'./runs/{log_name}/config.yaml'
        with open(config_file, "r") as file:
            saved_config = yaml.safe_load(file)
        print(f"Loaded configuration from {config_file}")

        
        # Override the command line arguments with values from the config file
        args.observation_type = saved_config.get('observation_type', args.observation_type)
        args.vol_constraint_type = saved_config.get('vol_constraint_type', args.vol_constraint_type)
        args.use_std_strain = saved_config.get('use_std_strain', args.use_std_strain)
        args.algorithm_name = saved_config.get('algorithm_name', args.algorithm_name)
        args.algorithm_config_file = saved_config.get('algorithm_config_file', args.algorithm_config_file)
        print(f"Configuration loaded: {saved_config}")
    ...
    print(f"Loading algorithm configuration from {args.algorithm_config_file}")
    ...
    print(f"Algorithm configuration loaded: {config}")
    ...
    print(f"Parameters defined: {params}")

    current_datetime = datetime.now().strftime("%Y%m%d_%H%M%S")
    train_critic_only = False  # if True, we freeze everything except the critic

    log_name = log_name if args.restart_run else f"{algorithm_name}_{current_datetime}_{os.getpid()}"
    ...
    if args.restart_run:
        model = model.load(args.restart_run, env=env)
        print("Model loaded.")

    # Save a checkpoint every 1000 steps
    checkpoint_callback = CheckpointCallback(
        save_freq=250_000 // num_cpu,
        save_path=f"./runs/{log_name}/checkpoints/",
    ...
    eval_callback = EvalCallback(eval_env,
                                 log_path=f'./runs/{log_name}/', 
                                 eval_freq=10_000 // num_cpu,
                                 deterministic=True,
                                 n_eval_episodes=10,
                                 render=False,
                                 best_model_save_path=f'./runs/{log_name}/checkpoints/',
                                 verbose=0)
    ...
    model.learn(100_000_000,
                ...
                reset_num_timesteps=not args.restart_run
                )

    # Save the model:
    model.save(f'./runs/{log_name}/checkpoints/final_model')
    if algorithm_name != 'PPO':
        model.save_replay_buffer(f"./runs/{log_name}/checkpoints/final_buffer")
